...
#
# Distilled from selkies-project/docker-nvidia-glx-desktop (MPL-2.0). The NVIDIA
# in-a-container Xorg setup below is kept verbatim from upstream -- it is the hard,
# fragile part (hex->decimal PCI BusID conversion, nvidia-xconfig generation, the
# ModeValidation overrides). Our changes vs upstream:
#   - removed the Selkies joystick interposer env (we use Sunshine's real uinput
#     devices, not the browser Gamepad API);
#   - removed the LibreOffice/Lutris PATH/LD_LIBRARY_PATH lines (not installed);
#   - replaced the KDE Plasma + Fcitx session with a lightweight XFCE session
#     (wallpaper + desktop icons + panel, xfwm4 WM).
...
# Install the NVIDIA userspace driver if either nvidia-xconfig OR the Xorg driver
# module is missing. On this lean image the NVIDIA Container Toolkit injects
# nvidia-xconfig + the GL/encode/fbc libraries but NOT the Xorg driver
# (nvidia_drv.so / libglxserver_nvidia.so). Gating only on nvidia-xconfig would
# skip the install, leaving Xorg to fall back to modesetting/llvmpipe -> NvFBC
# capture fails and Sunshine finds no encoder. So also trigger on nvidia_drv.so.
if ! command -v nvidia-xconfig >/dev/null 2>&1 || [ ! -e /usr/lib/xorg/modules/drivers/nvidia_drv.so ]; then
  ...
  if [ -f "/tmp/NVIDIA-Linux-${NVIDIA_DRIVER_ARCH}-${NVIDIA_DRIVER_VERSION}.run" ]; then
    ...
    # Tolerate a non-zero exit: the Container Toolkit injects some driver files
    # (e.g. EGL platform JSONs) read-only, so the installer reports "Device or
    # resource busy" on those and may exit non-zero even though nvidia_drv.so and
    # libglxserver_nvidia.so install fine. Don't let that abort the entrypoint.
    sudo ./nvidia-installer --silent \
        ...
        || echo 'nvidia-installer returned non-zero (often harmless: toolkit-injected read-only files); continuing'
    rm -rf /tmp/NVIDIA* && cd ~
  else
...
# Start the XFCE desktop session (wallpaper + desktop icons + panel, xfwm4 WM).
# Replaces the upstream KDE Plasma + Fcitx session. Runs within the dbus session
# the entrypoint is already wrapped in (dbus-run-session in supervisord.conf).
startxfce4 &
